...
class Song:
    ...
    def getSongID(self):
        ...
        input_url = "https://www.jiosaavn.com/api.php?__call=webapi.get&token={0}&type=song&includeMetaTags=0&ctx=web6dot0&api_version=4&_format=json&_marker=0".format(
            token
        )
    ...
    def getSong(self, song_id=None):
        ...
        try:
            song_json = [
                x for x in response.text.splitlines() if x.strip().startswith("{")
            ][0]
            ...
            self.dec_url = manager.get_dec_url(song["more_info"]["encrypted_media_url"])
    ...
    def downloadSong(self, album_name="songs", artist_name="Non-Artist"):
        manager = Manager()
        song = self.song_json[self.songID]
        album_name = song.get("more_info").get("album", "song")
        artist_name = ", ".join(
            [
                artist["name"]
        ...
        try:
            ...
            has_downloaded = manager.start_download(filename, location, self.dec_url)
            if has_downloaded:
                name = song.get("subtitle", "")
                try:
                    song["song"] = song["title"]
                    song["primary_artists"] = song["more_info"]["artistMap"][
                        "primary_artists"
                    ][0]["name"]
                    ...
                    print(e)
                manager.addtags(
                    location, song, playlist_name="", artist_name=artist_name
                )
                print("\n")
        except Exception as e:
            print("Download Error : {0}".format(e))
