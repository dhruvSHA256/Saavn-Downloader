...
class Playlist:
    ...
    def getPlaylistID(self, url=None):
        ...
        input_url = "https://www.jiosaavn.com/api.php?__call=webapi.get&token={0}&type=playlist&p=1&n=20&includeMetaTags=0&ctx=web6dot0&api_version=4&_format=json&_marker=0".format(
            token
        )
        try:
    ...
    def getPlaylist(self, playlistID=None):
        ...
        response = requests.get(
            "https://www.jiosaavn.com/api.php?__call=playlist.getDetails&listid={0}&contents=1&onlypids=true&api_version=4&_format=json&_marker=0&ctx=web6dot0".format(
                playlistID
            ),
    ...
    def downloadPlaylist(self):
        if self.playlistID is not None:
            manager = Manager()
            manager.downloadSongs(self.getPlaylist(), is_playlist=True)
